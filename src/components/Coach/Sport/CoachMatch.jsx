...
export default function CoachMatch() {
    ...
    const fetchData = () => {
    const data = { sportsId: id };
    const teamData = { coachId: userId, sportsId: id };

    // Fetch both matches and teams in parallel
    Promise.all([ApiService.allMatch(data), ApiService.allTeam(teamData)])
        .then(([matchRes, teamRes]) => {
            if (matchRes.data.success && teamRes.data.success) {
                console.log(matchRes,teamRes)
                const allMatches = matchRes.data.data;
                const coachTeams = teamRes.data.data;

                // Get only coach's team IDs
                const coachTeamIds = coachTeams.map(t => String(t._id));

                // Filter matches: only those that have at least one team belonging to coach
                const filteredMatches = allMatches.filter(m =>
                    Array.isArray(m.teamId) &&
                    m.teamId.length > 0 &&
                    m.teamId.some(team =>
                        coachTeamIds.includes(String(team._id || team))
                    )
                );
                ...
                if (!matchRes.data.success){
                    //  toast.error(matchRes.data.message);
                    
                if (!teamRes.data.success){
                    //  toast.error(teamRes.data.message);
                }
            }
        }
        })
                                            ...
                                            match.map((el, index) => (
                                                ...
                                                <span>No Image</span>
                                                        ...
                                                        <div className="property-content">
                                                            <div className="property-price"> {el.matchName}</div>
                                                            <h4 className="property-title">
                                                                

                                                            </h4>
                                                            <h4 className="property-title">
                                                                Teams: {el?.teamId?.map(t => t.teamName).join(" vs ")}
                                                            ...
                                                            <Link to={`/coach/coachteam/all/${el._id}`} className="btn btn-primary w-100">
                                                                View Details
                                                            </Link>
                                                            <br />
