...
export default function UpdateAnnounce() {
    ...
    const fetchData = () => {
        ...
        ApiService.singleAnnouncment({_id:id})
            .then((res) => {
                console.log(res);
                ...
                if (res.data.success) {
                    ...
                    setValue("leagueName", res.data.data.leagueName);
                    setValue("description", res.data.data.description);
                    setValue("maxTeams", res.data.data.maxTeams);


                    setValue("startDate", res.data.data.startDate
                        ? new Date(res.data.data.startDate).toISOString().split("T")[0]
                        : "")
                    setValue("endDate", res.data.data.endDate
                        ? new Date(res.data.data.endDate).toISOString().split("T")[0]
                        : "")
                        setValue("lastApplyDate", res.data.data.lastApplyDate
                        ? new Date(res.data.data.lastApplyDate).toISOString().split("T")[0]
                        : "")
    ...
    const handleForm = (data) => {
        ...
        ApiService.updateAnnouncment(formData)
            .then((res) => {
                if (res.data.success) {
                    setload(false)
                                ...
                                >
                                    <div className="col-lg-10">
                                        <div className="contact-form-wrapper">
                                            <h2 className="text-center mb-4">Update League Announcment</h2>
                                            <form
